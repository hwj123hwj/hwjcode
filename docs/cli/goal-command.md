...
工具实现：`packages/core/src/tools/local-time.ts`，CLI 与 VSCode 插件**自动共享**（都通过 `deepv-code-core` 包引用）。
...
- `packages/vscode-ui-plugin/package.json` — 增加 `deepv.openGoalWizard` 命令贡献
