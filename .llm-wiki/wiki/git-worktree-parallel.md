...
v1.1.64 移除 fail-fast 时引入了一个死锁 bug：当 parent abortSignal 在任务执行中
触发（如用户按 Ctrl+C），且有未调度的队列任务时，Promise 永不 resolve。

```
.finally(() => {
  active--;
  if (active === 0 && queueIndex >= queue.length) { resolve(); }  // ← abort 时 queueIndex < length，永不进
  else if (!this.abortSignal.aborted) { tryNext(); }              // ← aborted 时进不去
});
// → 死锁：active=0 但 queueIndex < length，两个条件都不满足
```

**修复**（v1.1.66）：resolve 条件增加 abort 检查：
...
### commit 失败上报修复（v1.1.66）

v1.1.64 的 commitAndCleanup 在 commit 失败时返回 `{ success: false, error }`，
但 `run()` 调用方只检查 `committed` 字段，**静默吞掉了 `success: false`**。

**修复后**（v1.1.66）：`run()` 在 `success === false` 时把 error 追加到 summary：
