/**
 * @license
 * Copyright 2026 Easy Code team
 * SPDX-License-Identifier: Apache-2.0
 *
 * Simplified Chinese catalog — the authoritative source of translation keys.
 * `TranslationKey` is derived from this object's keys, so `en.ts` MUST mirror
 * the exact same key set. Keys use a dot-namespaced style; `{name}` markers are
 * interpolated by `translate()`.
 */

export const zh = {
  // ── common ────────────────────────────────────────────────────────────────
  'common.cancel': '取消',
  'common.close': '关闭',
  'common.back': '返回',
  'common.save': '保存',
  'common.edit': '编辑',
  'common.copy': '复制',
  'common.delete': '删除',
  'common.remove': '移除',
  'common.rename': '重命名',
  'common.refresh': '刷新',
  'common.more': '更多',
  'common.settings': '设置',
  'common.local': '本地',
  'common.stop': '停止',
  'common.loading': '加载中…',
  'common.logout': '退出登录',

  // ── app boot ───────────────────────────────────────────────────────────────
  'app.booting': '正在启动 Easy Code…',

  // ── error boundary ──────────────────────────────────────────────────────────
  'error.boundaryTitle': '界面出错了',
  'error.retry': '重试',

  // ── relative / duration time ─────────────────────────────────────────────────
  'time.never': '从未',
  'time.justNow': '刚刚',
  'time.seconds': '{s} 秒',
  'time.minutes': '{m} 分钟',
  'time.hoursMinutes': '{h} 小时 {m} 分钟',
  'time.minutesAgo': '{m} 分钟前',
  'time.hoursAgo': '{h} 小时前',
  'time.daysAgo': '{d} 天前',

  // ── Feishu / Lark gateway ────────────────────────────────────────────────────
  'feishu.title': '飞书 / Lark 网关',
  'feishu.subtitle': '桌面版内置飞书网关，负责配置凭证与启停管理。',
  'feishu.fillAppIdSecret': '请填写 App ID 与 App Secret。',
  'feishu.saveFailed': '保存失败',
  'feishu.credsSaved': '凭证已验证并保存。',
  'feishu.qrStartFailed': '发起扫码失败',
  'feishu.qrFailed': '扫码失败',
  'feishu.qrSuccess': '扫码登录成功，凭证已保存。',
  'feishu.startFailed': '启动失败',
  'feishu.takeoverStarted': '已接管：关闭了 {n} 个外部网关并启动桌面版网关。',
  'feishu.gatewayStarted': '飞书网关已启动。',
  'feishu.gatewayStopped': '飞书网关已停止。',
  'feishu.externalKilled': '已关闭 {n} 个外部网关进程，现在可由桌面版启动并管理。',
  'feishu.credsCleared': '已清除飞书凭证。',
  'feishu.externalWarn':
    '检测到 {n} 个由 CLI 独立启动的飞书网关。一台机器只能运行一个网关，否则消息路由会混乱。建议关闭它们并改由桌面版管理。',
  'feishu.takeover': '关闭并接管',
  'feishu.gatewayRunning': '网关运行中',
  'feishu.gatewayStoppedStatus': '网关已停止',
  'feishu.notConfigured': '尚未配置',
  'feishu.uptime': '已运行 {time}',
  'feishu.bot': 'Bot：{name}',
  'feishu.platform': '平台：{name}',
  'feishu.platformFeishu': '飞书',
  'feishu.platformLabel': '平台',
  'feishu.owner': 'Owner：{id}…',
  'feishu.allowlist': '白名单：{n}',
  'feishu.bindings': '项目 / 群绑定',
  'feishu.noBindings': '暂无绑定。在飞书中 @ 机器人并发送消息，即可把当前群与一个项目自动绑定。',
  'feishu.p2pChat': '与机器人的私聊',
  'feishu.unknownChat': '未知会话',
  'feishu.noProject': '未绑定项目',
  'feishu.p2pTag': '私聊',
  'feishu.activeTag': '活跃',
  'feishu.runningTag': '运行中',
  'feishu.stopGateway': '停止网关',
  'feishu.startGateway': '启动网关',
  'feishu.reconfigure': '重新配置',
  'feishu.qrLogin': '扫码登录',
  'feishu.qrLoginDesc': '推荐 · 使用飞书 App 扫码，自动完成应用创建与授权，无需任何开发者配置。',
  'feishu.manualEntry': '手动输入凭据',
  'feishu.manualEntryDesc': '已有飞书应用？填入 App ID 与 App Secret 直接接入。',
  'feishu.qrHint': '使用 {platform} 扫描二维码并授权创建应用。',
  'feishu.verifyCode': '验证码',
  'feishu.openInBrowser': '在浏览器打开',
  'feishu.qrStarting': '正在发起扫码…',
  'feishu.appSecretPlaceholder': '应用密钥',
  'feishu.verifyAndSave': '验证并保存',
  'feishu.lastError': '最近错误：{msg}',
  'feishu.detailsLog': '详情输出',
  'feishu.detailsLogSize': '{n} 字符',
  'feishu.detailsLogEmpty': '暂无输出。网关启动后，这里会实时显示其日志（WebSocket 连接、权限校验、错误等）。',

  // ── feishu authorization management（owner / 白名单图形化配置） ───────────────
  'feishu.auth.title': '授权管理',
  'feishu.auth.ownerLabel': 'Owner（拥有者）',
  'feishu.auth.ownerNone': '未设置',
  'feishu.auth.verified': '已确认',
  'feishu.auth.unverified': '待首次私聊确认',
  'feishu.auth.edit': '编辑',
  'feishu.auth.save': '保存',
  'feishu.auth.ownerPlaceholder': 'Bot 应用 open_id（ou_…）',
  'feishu.auth.ownerHint':
    'Owner 是被授权驱动 Bot 的 open_id。应填 Bot 应用自身空间的 open_id（与 Bot 私聊时事件里的 sender open_id）；手动设置会标记为已确认，自动绑定（TOFU）将不再生效。',
  'feishu.auth.allowlistLabel': '授权用户白名单',
  'feishu.auth.allowlistEmpty': '暂无额外授权用户。',
  'feishu.auth.allowPlaceholder': '要授权的 open_id（ou_…）',
  'feishu.auth.add': '添加',
  'feishu.auth.remove': '移除',
  'feishu.auth.restartHint': '网关运行中——改动需重启网关后才对运行中的会话生效。',
  'feishu.auth.cmdFailed': '命令执行失败',
  'feishu.auth.working': '处理中…',

  // ── login ────────────────────────────────────────────────────────────────────
  'login.failed': '登录失败',
  'login.browserFailed': '无法启动浏览器登录',
  'login.tagline': '登录以开始 — 与 CLI 共享同一登录凭证',
  'login.browser': '浏览器登录',
  'login.browserHint': '已在浏览器打开登录页（{url}）。完成后将自动进入。',
  'login.or': '或',
  'login.apiKeyPlaceholder': '粘贴你的 API Key',
  'login.apiKeyLogin': '用 API Key 登录',
  'login.useCustomModels': '使用自定义模型继续',
  'login.addCustomModel': '添加自定义模型',
  'login.customModelHint': '无需登录，使用你自己的 API Key（与 CLI 共享配置）。',
  'login.noCustomModels': '尚未配置可用的自定义模型，请先添加一个。',

  // ── agent backends ─────────────────────────────────────────────────────────────
  'agent.easyCodeHint': '内置 Easy Code 后端（默认）',
  'agent.claudeCodeHint': '驱动本机已安装的 Claude Code（claude）',
  'agent.codexHint': '驱动本机已安装的 Codex（codex）',

  // ── new session dialog ──────────────────────────────────────────────────────────
  'newSession.title': '新建会话',
  'newSession.subtitle': '每个会话拥有独立的工作目录与上下文，可并行运行。',
  'newSession.pickDirError': '请选择项目目录',
  'newSession.environment': '环境',
  'newSession.type': '类型',
  'newSession.typeProject': '项目',
  'newSession.typeChat': '闲聊',
  'newSession.typeProjectHint': '绑定一个工作目录',
  'newSession.typeChatHint': '不绑定目录，直接对话',
  'newSession.projectDir': '项目目录',
  'newSession.projectDirOptional': '项目目录（可选）',
  'newSession.pickFolderPlaceholder': '选择一个文件夹…',
  'newSession.pickFolderOptionalPlaceholder': '留空则直接闲聊，不绑定目录',
  'newSession.browse': '浏览…',
  'newSession.recentProjects': '最近的项目',
  'newSession.permissionMode': '权限模式',
  'newSession.externalAgentNote': '{agent} 将以其自身的权限策略运行；需要确认时会弹出授权请求。',
  'newSession.start': '开始会话',

  // ── permission modes (mirrors PERMISSION_MODES ids) ──────────────────────────────
  'permMode.default': '每次询问',
  'permMode.default.hint': '每次编辑/命令前询问',
  'permMode.yolo': 'YOLO 自动接受',
  'permMode.yolo.hint': '自动接受所有编辑与命令，不再询问',

  // ── permission dialog ────────────────────────────────────────────────────────────
  'permission.title': '需要你的批准',

  // ── ask_user_question (multi-choice cards) ───────────────────────────────────────
  'ask.title': '请回答以下问题',
  'ask.other': '其他（自定义输入）…',
  'ask.otherPlaceholder': '输入你的回答…',
  'ask.submit': '提交',
  'ask.skip': '跳过',
  'ask.multiHint': '可多选',
  'ask.preview': '预览',

  // ── diff (shared edit/write verbs + diff pane) ───────────────────────────────────
  'diff.edit': '编辑',
  'diff.write': '写入',
  'diff.reviewPrompt': '请处理以下针对当前改动的代码评审意见：\n\n',
  'diff.selfReviewPrompt':
    '请自审当前未提交的改动，找出编译错误、逻辑错误、安全问题或明显 bug（忽略风格/lint），并直接修复。',
  'diff.selfReviewTitle': '自审改动',
  'diff.submitComments': '提交 {n} 条评论',
  'diff.noChanges': '无未提交改动',
  'diff.pickFile': '选择一个文件查看 diff',
  'diff.commentPlaceholder': '写下评审意见，回车提交…',
  'diff.addCommentTitle': '点击行添加评论',

  // ── prompt bar ────────────────────────────────────────────────────────────────────
  'prompt.placeholder': '输入指令，@ 引用文件，/ 使用命令…',
  'prompt.busyPlaceholder': '发送将打断当前任务并开始新对话…',
  'prompt.externalAgentTitle': '驱动此会话的外部 agent',
  'prompt.cwdHint': '工作文件夹在会话创建时固定。如需更换，请新建一个会话。',
  'prompt.defaultModel': '默认模型',
  'prompt.contextUsage': '上下文用量',
  'prompt.ctxUsed': '已用',
  'prompt.ctxTotal': '总计',
  'prompt.ctxRemaining': '剩余',
  'prompt.addAttachment': '添加附件 / 图片',
  'prompt.hint': 'Enter 发送 · Shift+Enter 换行 · {paste} 粘贴图片 · 点击回形针添加附件',

  // ── thinking（扩展思考 / 推理强度）─────────────────────────────────────────
  'thinking.title': '思考',
  'thinking.toggleDesc': '可针对更复杂的任务进行思考',
  'thinking.effort.title': '思考强度',
  'thinking.help': '强度越高，回答越深入，但耗时更长、消耗额度更快。',
  'thinking.default': '默认',
  'thinking.effort.low': '低',
  'thinking.effort.medium': '中',
  'thinking.effort.high': '高',
  'thinking.effort.max': '最高',
  'thinking.state.off': '关闭',
  'thinking.state.auto': '自动',

  // ── settings · general + custom models ─────────────────────────────────────────────
  'settings.title': '设置',
  'settings.subtitlePre': '与 CLI 共用 ',
  'settings.subtitlePost': ' 下的配置文件，修改后将在下次新建会话时生效。',
  'settings.tabGeneral': '通用',
  'settings.tabModels': '自定义模型',
  'settings.navMcp': 'MCP 服务器',
  'settings.backToApp': '返回应用',
  'settings.searchPlaceholder': '搜索设置…',
  'settings.searchNoMatch': '没有匹配的设置',
  'settings.groupPersonal': '个人',
  'settings.groupIntegration': '集成',
  'settings.navAppearance': '外观',
  'settings.navPersonalization': '个性化',
  'settings.navComputerUse': '电脑控制',
  'settings.navGeneralModel': '通用模型设置',
  'settings.groupArchived': '已归档',
  'settings.navArchivedChats': '已归档对话',
  'settings.archivedSearchPlaceholder': '搜索已归档对话…',
  'settings.archivedAllChats': '全部对话',
  'settings.archivedAllProjects': '全部项目',
  'settings.archivedDeleteAll': '全部删除',
  'settings.archivedEmpty': '暂无已归档对话',
  'settings.archivedNoMatch': '没有匹配的已归档对话',
  'settings.archivedChatCount': '{n} 个对话',
  'settings.archivedUnarchiveAll': '全部取消归档',
  'settings.archivedDeleteProjectShort': '删除该组全部对话',
  'settings.archivedDeleteProject': '「{name}」下的全部已归档对话',
  'settings.archivedDeleteCountConfirm': '确定永久删除这 {n} 个对话？',
  'settings.groupAbout': '关于',
  'settings.navAbout': '关于',
  'settings.aboutDesktop': 'Easy Code 桌面版',
  'settings.aboutCliCore': '内置后端 (easycode-cli-core)',
  'settings.aboutOs': '操作系统',
  'settings.aboutCopy': '复制信息',
  'settings.aboutCopied': '已复制',
  'settings.customInstructions': '自定义指令',
  'settings.customInstructionsDesc': '为此主机上的所有任务提供额外说明和上下文。',
  'settings.customInstructionsPlaceholder':
    '例如：始终用中文回复；提交信息使用约定式提交；优先使用函数式风格…',
  'settings.customInstructionsHint': '保存后在新建会话时生效，或重启主程序后对所有会话生效。',
  'settings.language': '显示语言',
  'settings.langZh': '简体中文',
  'settings.langEn': 'English',
  'settings.theme': '外观主题',
  'settings.themeSystem': '跟随系统',
  'settings.themeLight': '浅色',
  'settings.themeDark': '深色',
  'settings.themeDesc': '选择浅色或深色，或跟随系统设置。',
  'settings.terminalShell': '集成终端 Shell',
  'settings.terminalShellDesc': '选择要在集成终端中打开的 Shell。',
  'settings.shellDefault': '默认',
  'settings.shellPowershell': 'PowerShell',
  'settings.shellCmd': 'Command Prompt',
  'settings.shellGitbash': 'Git Bash',
  'settings.shellWsl': 'WSL',
  'settings.shellBash': 'bash',
  'settings.shellZsh': 'zsh',
  'settings.shellFish': 'fish',
  'settings.shellUnavailable': '未安装',
  'settings.replyLanguage': '回复语言',
  'settings.replyLanguagePlaceholder': '例如 中文 / English（留空使用模型默认）',
  'settings.replyLanguageDesc': '模型回复时优先使用的语言。',
  'settings.defaultModel': '全局默认模型',
  'settings.defaultModelAuto': '自动（后端默认）',
  'settings.defaultModelDesc': '新建会话时默认使用的模型，省去每次手动选择。',
  'settings.overrideAutoDefault': '默认（自动）',
  'settings.overrideInherit': '继承会话模型',
  'settings.overrideCompression': '压缩模型',
  'settings.overrideCompressionDesc': '上下文压缩时使用的模型。不设置则走内置默认。',
  'settings.overrideCodeExpert': 'Code Expert 子代理模型',
  'settings.overrideCodeExpertDesc': '代码分析子代理使用的模型。不设置则继承当前会话模型。',
  'settings.overrideVerification': 'Verification 子代理模型',
  'settings.overrideVerificationDesc': '验证子代理使用的模型。不设置则继承当前会话模型。',
  'settings.projectMemory': '项目记忆',
  'settings.memoryAll': '全部',
  'settings.memoryDeepvOnly': '仅 DEEPV.md',
  'settings.memoryNone': '不加载',
  'settings.memoryAllHint': 'DEEPV.md + AGENTS.md',
  'settings.memoryDeepvOnlyHint': '只加载 DEEPV.md',
  'settings.memoryNoneHint': '忽略项目记忆文件',
  'settings.projectMemoryDesc': '新建会话时加载哪些项目记忆文件（{hint}）。',
  'settings.healthyUse': '健康使用提醒',
  'settings.healthyUseDesc': '长时间连续使用时显示休息提醒。',
  'settings.computerUse': '电脑控制（操作本机）',
  'settings.computerUseDesc':
    '允许智能体查看你的屏幕，并用真实的鼠标键盘操作任意应用。默认关闭，对新建会话生效。请仅在可信任务中使用——操作时请留意，若它点错窗口请及时停止。推荐搭配 Claude Sonnet 4.6 等同等级多模态模型使用。',
  'settings.computerUseUnavailable': '当前平台不支持。',
  'settings.computerUseExperimental':
    '实验性功能。每一步都会把屏幕截图发送给模型，因此单个任务可能很快消耗较多积分——请尽量缩短任务并留意用量。',
  'settings.computerUseMacPerms':
    'macOS 上请在 系统设置 → 隐私与安全性 中授予 Easy Code 两项权限，并重启应用：开启「屏幕录制」（用于查看屏幕）和「辅助功能」（用于移动鼠标和输入）。未授权时截图会是空白、点击也不会生效。',
  'computerUse.bannerActive': 'Easy Code 正在控制你的电脑',
  'computerUse.bannerHint': '它正在你的屏幕上移动鼠标并输入 —— 按 Esc 可随时停止。',
  'computerUse.stop': '停止',
  'settings.saved': '已保存',
  'settings.errName': '请填写名称',
  'settings.errBaseUrl': '请填写 Base URL',
  'settings.errApiKey': '请填写 API Key',
  'settings.errModelId': '请填写模型 ID',
  'settings.saveFailed': '保存失败',
  'settings.noModels': '还没有自定义模型',
  'settings.disabled': '已禁用',
  'settings.addModel': '添加自定义模型',
  'settings.name': '名称',
  'settings.namePlaceholder': '例如 My GPT-4o',
  'settings.provider': '协议',
  'settings.apiKeyPlaceholder': 'sk-… 或 ${ENV_VAR}',
  'settings.modelId': '模型 ID',
  'settings.contextWindow': '上下文窗口（可选，tokens）',
  'settings.contextWindowPlaceholder': '例如 200000',
  'settings.enableModel': '启用此模型',

  // ── settings · MCP servers ───────────────────────────────────────────────
  'settings.mcpDesc': '管理 MCP（Model Context Protocol）服务器。配置与 CLI 共用，修改将在下次新建会话时生效。',
  'settings.noMcpServers': '还没有 MCP 服务器',
  'settings.addMcpServer': '添加 MCP 服务器',
  'settings.mcpNamePlaceholder': '例如 fetch / context7',
  'settings.mcpTransport': '传输方式',
  'settings.mcpTransportStdio': '本地进程 (stdio)',
  'settings.mcpTransportSse': 'SSE',
  'settings.mcpTransportHttp': 'HTTP',
  'settings.mcpCommand': '启动命令',
  'settings.mcpCommandPlaceholder': '例如 npx / uvx / node',
  'settings.mcpArgs': '参数（每行一个）',
  'settings.mcpArgsPlaceholder': '-y\n@modelcontextprotocol/server-fetch',
  'settings.mcpEnv': '环境变量（每行 KEY=VALUE）',
  'settings.mcpEnvPlaceholder': 'API_KEY=${MY_API_KEY}',
  'settings.mcpCwd': '工作目录（可选）',
  'settings.mcpUrl': '服务器 URL',
  'settings.mcpHeaders': '请求头（每行 KEY: VALUE）',
  'settings.mcpHeadersPlaceholder': 'Authorization: Bearer ${TOKEN}',
  'settings.mcpTimeout': '超时（毫秒，可选）',
  'settings.mcpTimeoutPlaceholder': '例如 30000',
  'settings.mcpTrust': '信任此服务器（跳过工具确认）',
  'settings.mcpDescription': '描述（可选）',
  'settings.mcpEnableServer': '启用此服务器',
  'settings.mcpEnabled': '已启用',
  'settings.mcpDisabled': '已禁用',
  'settings.errMcpName': '请填写服务器名称',
  'settings.errMcpCommand': '请填写启动命令',
  'settings.errMcpUrl': '请填写服务器 URL',

  // ── tool call status ─────────────────────────────────────────────────────────────────
  'tool.status.pending': '等待',
  'tool.status.in_progress': '执行中',
  'tool.status.completed': '完成',
  'tool.status.failed': '失败',

  // ── grouped tool calls (collapsible runs) ──────────────────────────────────────────────
  'tool.ranCount': '已运行 {n} 条命令',
  'tool.ranItem': '已运行 {title}',

  // ── tool result summaries (single-line description, VSCode-plugin style) ───────────────
  'tool.sum.read': '读取 {name}，{n} 行',
  'tool.sum.readLines': '读取 {name}，第 {range} 行',
  'tool.sum.readFiles': '读取 {n} 个文件',
  'tool.sum.readName': '读取 {name}',
  'tool.sum.found': '找到 {n} 处匹配："{q}"',
  'tool.sum.foundNoQuery': '找到 {n} 处匹配',
  'tool.sum.foundFiles': '找到 {n} 个文件："{q}"',
  'tool.sum.noMatches': '未找到匹配："{q}"',
  'tool.sum.noMatchesNoQuery': '未找到匹配',
  'tool.sum.listed': '列出 {n} 项',
  'tool.sum.edited': '编辑 {name}',
  'tool.sum.created': '创建 {name}',
  'tool.sum.deleted': '删除 {name}',
  'tool.sum.exit': '退出码 {code}',
  'diff.moreLines': '… 还有 {n} 行',

  // ── panes ──────────────────────────────────────────────────────────────────────────────
  'pane.chat': '对话',
  'pane.diff': '改动',
  'pane.plan': '计划',
  'pane.tasks': '任务',
  'pane.terminal': '终端',
  'pane.file': '文件',

  // ── session run status ───────────────────────────────────────────────────────────────────
  'status.idle': '空闲',
  'status.starting': '启动中',
  'status.thinking': '思考中',
  'status.needs_approval': '等待批准',
  'status.error': '错误',
  'status.exited': '已退出',

  // ── session view ─────────────────────────────────────────────────────────────────────────
  'session.emptyTitle': '选择左侧会话，或新建一个会话开始',
  'session.emptyPrompt': '我们来做点什么？',
  'session.emptyPlaceholder': '随便聊聊，或描述一个任务…',
  'session.emptySend': '发送',
  'session.emptyHint': '直接输入即可开始，可在下方选择工作目录与权限模式',
  'session.emptyChatTarget': '直接闲聊',
  'session.emptyRecent': '最近的项目',
  'session.emptyPickFolder': '选择文件夹…',
  'session.resume': '恢复会话',
  'session.views': '视图',
  'session.openWith': '用其他程序打开',
  'session.openWithEmpty': '未检测到可用程序',
  'session.openIn': '用 {name} 打开',
  'session.openWithSwitch': '切换程序',

  // ── chat density ──────────────────────────────────────────────────────────────────────────
  'density.title': '视图密度',
  'density.summary': '摘要',
  'density.normal': '正常',
  'density.verbose': '详细',

  // ── sidebar ────────────────────────────────────────────────────────────────────────────────
  'sidebar.newSession': '新建会话',
  'sidebar.newChat': '新建对话',
  'sidebar.search': '搜索',
  'sidebar.active': '进行中',
  'sidebar.all': '全部',
  'sidebar.archived': '已归档',
  'sidebar.searchPlaceholder': '搜索会话…',
  // ── 搜索命令面板 ──────────────────────────────────────────────────────────
  'search.placeholder': '搜索对话或运行命令',
  'search.chats': '对话',
  'search.suggested': '建议',
  'search.newChat': '新建对话',
  'search.openFolder': '打开文件夹',
  'search.noResults': '未找到与「{query}」匹配的结果',
  'sidebar.noSessions': '暂无会话',
  'sidebar.chats': '闲聊',
  'sidebar.chatsFolder': '闲聊',
  'sidebar.projects': '项目',
  'sidebar.dblClickRename': '双击重命名',
  'sidebar.archive': '归档',
  'sidebar.unarchive': '取消归档',
  'sidebar.delete': '删除',
  'sidebar.deleteTitle': '删除会话',
  'sidebar.deleteConfirm': '确定永久删除「{title}」？',
  'sidebar.deleteWarning': '此操作不可恢复，该会话及其历史记录将被彻底移除。',
  'sidebar.collapse': '收起侧边栏',
  'sidebar.expand': '展开侧边栏',
  'sidebar.resize': '拖动调整侧边栏宽度',
  'sidebar.loggedIn': '已登录',
  'sidebar.customModelMode': '自定义模型模式',
  'sidebar.exitCustomModelMode': '退出自定义模型模式',
  'sidebar.feishu': '飞书网关',
  'sidebar.feishuRunning': '飞书网关（运行中）',

  // ── chat pane ───────────────────────────────────────────────────────────────────────────────
  'chat.emptyTitle': '开始与 Easy Code 对话',
  'chat.worksIn': '它会在 {cwd} 中工作',
  'chat.restoring': '正在恢复会话…',
  'chat.aiResponding': 'AI 正在响应',
  'chat.attachedImage': '附带图片',
  'chat.thought': '思考',
  'chat.rewindTitle': '回退到此处（rewind）',

  // ── plan / tasks / terminal / file panes ───────────────────────────────────────────────────────
  'plan.empty': '尚无计划',
  'todo.title': '任务清单',
  'todo.stepLabel': '第 {step}/{total} 步',
  'tasks.summary': '{running} 进行中 / {total} 总计',
  'tasks.empty': '暂无工具调用',
  'terminal.noOutput': '（暂无命令输出）',
  'terminal.backendLog': '后端日志',
  'terminal.none': '（无）',
  'file.empty': '点击工具调用中的文件路径以查看',

  // ── 工作区布局（Codex 风格右侧栏 + 底部终端） ─────────────────────────────
  'workspace.toggleBottom': '切换底部栏',
  'workspace.toggleRight': '切换侧边栏',
  'workspace.review': '审阅',
  'workspace.terminal': '终端',
  'workspace.browser': '浏览器',
  'workspace.files': '文件',
  'workspace.sidechat': '侧边对话',

  // ── 文件浏览器（类 VSCode 多标签） ──────────────────────────────────────────
  'files.title': '文件',
  'files.noProject': '打开一个项目会话以浏览其文件',
  'files.empty': '从左侧选择一个文件',
  'files.noOpenTabs': '尚未打开文件 — 从左侧选择一个',
  'files.openIn': '打开方式',
  'files.openInFolder': '在文件管理器中显示',
  'files.openInTerminal': '在终端中打开',
  'files.preview': '预览',
  'files.source': '源码',
  'files.explorer': '资源管理器',
  'files.binary': '二进制或非文本文件 — 无法预览',
  'files.loadFailed': '无法读取此文件',
  'files.closeTab': '关闭',
  'files.noIde': '未检测到外部编辑器',
  'files.searchPlaceholder': '搜索文件…',
  'files.searchClear': '清除',
  'files.searchNoResults': '未找到匹配的文件',
  'files.searchGoogle': '使用 Google 搜索',
  'files.copy': '复制',
  'files.selectAll': '全选',

  // ── 内置浏览器 ───────────────────────────────────────────────────────────────
  'browser.title': '浏览器',
  'browser.placeholder': '输入网址或搜索…',
  'browser.back': '后退',
  'browser.forward': '前进',
  'browser.reload': '刷新',
  'browser.home': '主页',
  'browser.openExternal': '在系统浏览器中打开',
  'browser.blank': '在上方输入网址开始浏览',
  'browser.newTab': '新建标签页',
  'browser.newTabTitle': '新标签页',
  'browser.closeTab': '关闭标签页',
  // 会话内 URL 的右键菜单
  'link.openHere': '在浏览器中打开',
  'link.openExternal': '在外部浏览器中打开',
  'link.copy': '复制链接',

  // ── 底部终端 ─────────────────────────────────────────────────────────────────
  'terminalPanel.title': '终端',
  'terminalPanel.new': '新建终端',
  'terminalPanel.close': '关闭终端',
  'terminalPanel.clear': '清空',
  'terminalPanel.tab': '终端 {n}',
  'terminalPanel.placeholder': '输入命令，回车执行…',
  'terminalPanel.starting': '正在启动 shell…',
  'terminalPanel.exited': '[进程已退出，代码 {code}]',

  // ── 侧边对话 ─────────────────────────────────────────────────────────────────
  'sidechat.title': '侧边对话',
  'sidechat.empty': '一个独立于主会话的快速草稿对话。',
  'sidechat.starting': '正在启动…',

  // ── version update ───────────────────────────────────────────────────────────────────────────
  'update.available': '发现新版本 {version}',
  'update.currentVersion': '当前版本 v{version}',
  'update.updateNow': '立即更新',
  'update.later': '稍后',
  'update.skip': '跳过此版本',
  'update.downloading': '正在下载更新…',
  'update.downloaded': '更新已下载完成',
  'update.installNow': '立即安装',
  'update.installingTitle': '正在启动安装',
  'update.installingMac': '安装包已打开，请将 Easy Code 拖入“应用程序”以完成更新',
  'update.installingWin': '即将退出并运行安装程序…',
  'update.failed': '更新失败',
  'update.retry': '重试',
  'update.minimize': '最小化（后台继续下载）',
  'update.expand': '展开更新进度',
  // settings → 软件更新
  'update.section': '软件更新',
  'update.checkNow': '检查更新',
  'update.checking': '正在检查…',
  'update.upToDate': '已是最新版本（v{version}）',
  'update.newAvailable': '有可用更新：{version}',
  'update.checkFailed': '检查失败，请稍后重试',

  // ── 代码块 ───────────────────────────────────────────────────────────────────
  'code.copy': '复制代码',
  'code.copied': '已复制',

  // ── mermaid diagrams ─────────────────────────────────────────────────────────────────────────
  'mermaid.copy': '复制源码',
  'mermaid.copied': '已复制',
  'mermaid.download': '下载图片',
  'mermaid.downloaded': '已保存',
  'mermaid.zoom': '放大查看',
  'mermaid.viewSource': '查看源码',
  'mermaid.viewDiagram': '查看图表',
  'mermaid.rendering': '正在渲染图表…',
  'mermaid.error': '图表渲染失败',
} as const;
