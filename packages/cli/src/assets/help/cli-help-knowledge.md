...
> 本知识库包含 Easy Code CLI 的所有命令和功能详细说明，供智能帮助系统使用。
...
### Q4: 如何升级 Easy Code CLI？
**A:** 使用 `easycode -u` 命令进行升级。


---

### Q5: 如何让 CLI 感知 VS Code 中打开的文件或选中的代码？
**A:** Easy Code CLI 可以通过 VS Code 扩展与编辑器进行深度集成。
1. **安装扩展：** 在 VS Code 扩展市场搜索并安装 `Easy Code Companion` 扩展。
2. **启动 CLI：** 在 VS Code 的内置终端中启动 Easy Code CLI。
...
**位置选择：**
- 全局命令：`~/.easycode-user/commands/`（所有项目可用）
- 项目命令：`<项目根目录>/.easycode/commands/`（仅当前项目）

**文件格式示例：**
```toml
# 文件：~/.easycode-user/commands/git/commit.toml
# 命令名：/git:commit
...
**位置选择：**
- 全局配置：`~/.easycode-user/settings.json`（所有项目可用）
- 项目配置：`<项目根目录>/.easycode/settings.json`（仅当前项目）
...
**A:** 是的，Easy Code 也支持 VSCode 扩展！
你可以在 VSCode 扩展市场中搜索 **Easy Code for VSCode** 在线安装，即可在 VSCode 中使用图形化界面版本。

**安装方式：**
1. 打开 VSCode
2. 进入扩展市场（快捷键：`Ctrl+Shift+X` 或 `Cmd+Shift+X`）
3. 搜索 `Easy Code for VSCode`
4. 点击安装
...
**A:** Easy Code 支持配置 OpenAI 兼容格式和 Anthropic Claude API 格式的自定义模型。

#### 📝 配置文件位置

自定义模型配置保存在：`~/.easycode-user/custom-models.json`

**为什么使用独立文件？**
- 避免与 `settings.json` 产生冲突
- 防止云端模型更新覆盖自定义配置
- 支持多个 Easy Code 实例并发运行
...
- **重启生效**：修改配置后需要重启 Easy Code
- **Token 计费**：自定义模型不消耗 EasyCode 积分，直接向提供商付费
...
**A:** 使用 `/feishu` 子命令系列，可以将 Easy Code 直接作为一个智能群聊或私聊 Bot 接入到你的飞书工作区：
1. **启动配置：** 输入 `/feishu`，选择 QR 扫码自动建应用或手动配置。
2. **凭据安全：** 凭证经过 AES-256-GCM 高强度加密并存放在 `~/.easycode-user/` 全局目录下，自动采用 0o600 权限锁闭。
3. **日常管控：**
...
### `/init` - 初始化项目上下文
自动分析项目并生成 `EASYCODE.md` 文件，提供项目上下文。
...
**注意：** 如果 `EASYCODE.md` 已存在，此命令则无法执行。
...
### `/memory` - 记忆管理
管理 AI 的指令上下文（从 `EASYCODE.md` 文件加载）。

**子命令：**
- `add <text>` - 添加文本到 AI 记忆
- `show` - 显示当前加载的完整分层记忆
- `refresh` - 重新加载所有  `EASYCODE.md` 文件
...
**记忆文件层次（优先级递增）：**
1. 全局：`~/.easycode-user/EASYCODE.md`
2. 项目根及父目录：`<项目路径>/EASYCODE.md`
3. 子目录：项目内各子目录的 `EASYCODE.md`
...
### `/config` - 统一配置菜单
打开交互式设置面板，让您可以直观地配置 Easy Code 的各种设置。常用的设置如主题、模型、Agent 风格等都在这里了。
...
**持久化：** 偏好保存到 `~/.easycode-user/settings.json`
...
### `/feishu` - 飞书机器人管理
管理 Easy Code 与飞书 (Feishu) / Lark 的机器人长连接及接入。
...
**位置：**
- 全局：`~/.easycode-user/commands/`
- 项目：`<项目根>/.easycode/commands/`

#### 文件格式：TOML（`.toml` 扩展名）

#### 命名：文件路径决定命令名
- `~/.easycode-user/commands/test.toml` → `/test`
- `<项目>/.easycode/commands/git/commit.toml` → `/git:commit`
...
**示例：**
```toml
# ~/.easycode-user/commands/changelog.toml
...
### 配置层次（优先级递增）
1. **默认值**：硬编码默认值
2. **用户设置**：`~/.easycode-user/settings.json`（全局）
3. **项目设置**：`<项目根>/.easycode/settings.json`（项目专用）
...
- `--output-format <format>`：非交互模式的输出格式
  ...
  - 示例：`easycode --output-format stream-json --yolo "你的prompt内容"`
- `--yolo`：自动批准所有工具调用
...
```bash
# 标准输出
easycode --prompt "解释 main.py" -a

# JSONL 格式（一行一个 JSON 对象）
easycode --output-format stream-json --yolo "分析代码并生成改进建议" @src/

# 与其他工具组合
easycode --output-format stream-json --yolo "生成代码" @schema.json | jq '.content'
```

**其他参数示例：**
```bash
easycode --model gemini-2.0-flash-exp --debug
easycode --checkpointing --sandbox
easycode -e my-extension -e another
```
...
### 层次结构（优先级递增）
1. **全局：** `~/.easycode-user/GEMINI.md`（所有项目）
...
### 启用方式
- 命令行：`easycode --sandbox` 或 `easycode -s`
...
### 自定义沙箱
在项目根创建 `.easycode/sandbox.Dockerfile`：
```dockerfile
...
- `strict`：严格模式
- `<profile_name>`：自定义 profile（`.easycode/sandbox-macos-<profile_name>.sb`）
...
### 场景 4：配置 MCP 服务器
编辑 `.easycode/settings.json` 或 `~/.easycode-user/settings.json`：
```json
{
  "mcpServers": {
    "database": {
...
### 场景 5：创建自定义命令
创建 `~/.easycode-user/commands/docs/generate.toml`：
```toml
...
### 问题：自定义命令不显示
**检查：**
1. 文件位置：`~/.easycode-user/commands/` 或 `<项目>/.easycode/commands/`
2. 文件扩展名：必须是 `.toml`
...
这份知识库涵盖了 Easy Code CLI 的所有核心功能。如果你有任何问题，请随时询问！
...
**A:** 使用 `easycode checkpoint clean` 命令清理所有历史检查点。

**命令用法：**
```bash
# 清理所有 checkpoint 历史（会提示确认）
easycode checkpoint clean

# 预览将要删除的内容（不实际删除）
easycode checkpoint clean --dry-run

# 跳过确认直接删除
easycode checkpoint clean --force
```

**别名：** 可以使用 `easycode cp clean` 作为简写。
...
📊 Checkpoint History Summary:
   Projects: 73
   Total Size: 4.18 GB
   Location: C:\Users\username\.easycode-user\history
...
Skills 系统数据存储在：
- **目录：** `~/.easycode-user/skills/` 和 `~/.easycode-user/marketplace/`
- **配置：** `~/.easycode-user/skills/settings.json`
- **已安装记录：** `~/.easycode-user/skills/installed_plugins.json`
- **备份：** `~/.easycode-user/skills/backups/`
...
**A:** Easy Code 提供了强大的 Hooks 系统，允许你用任何脚本语言（Python、Bash、PowerShell、Batch 等）在关键操作点插入自定义脚本来监控、控制和修改系统行为。
...
```
配置：`"command": "bash .easycode/hooks/security.sh"`
...
sys.exit(0)
```
配置：`"command": "python .easycode/hooks/security.py"`
...
```
配置：`"command": "powershell -ExecutionPolicy Bypass .easycode/hooks/security.ps1"`

**第 3 步：配置到 `.easycode/settings.json`**
```json
{
  "hooks": {
    "BeforeTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "python .easycode/hooks/security.py",
            "timeout": 5000
          }
        ]
      }
...
**第 4 步：测试**
```bash
echo '{"tool_name":"delete_file"}' | python .easycode/hooks/security.py
...
```
使用：`USER_ROLE=viewer easycode-cli`
...
```json
{
  "hooks": {
    "BeforeTool": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "python .easycode/hooks/hook.py",
...
### 什么是 LSP？
**LSP** 是由微软、Google 和 RedHat 共同推出的行业标准。它将代码分析逻辑与编辑器分离。通过 LSP，Easy Code 可以调用各语言官方最专业的分析引擎（如 `typescript-language-server`, `pyright`, `rust-analyzer` 等），为 AI 提供极致的代码感知力。
...
- **自动安装**：如果你的电脑没装对应的 LSP 服务端，Easy Code 会**自动在后台下载并安装**到隔离目录。
...
### 核心功能与工具集
Easy Code 提供了一套完整的语义化工具，帮助 AI 实现“手术刀”级的代码操作：
