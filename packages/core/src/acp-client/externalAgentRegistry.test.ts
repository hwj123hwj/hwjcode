...
describe('resolveExternalAgentSpec', () => {
  it('resolves claude-code to the claude-agent-acp npx bridge', () => {
    ...
    expect(spec.args).toEqual(['-y', '@agentclientprotocol/claude-agent-acp@0.52.0']);
  });

  it('resolves codex to the agentclientprotocol codex-acp npx bridge', () => {
    const spec = resolveExternalAgentSpec('codex', {});
    expect(spec.type).toBe('codex');
    expect(spec.label).toBe('Codex');
    expect(spec.command).toBe('npx');
    expect(spec.args).toEqual(['-y', '@agentclientprotocol/codex-acp@1.0.2']);
  });
