...
const translations = {
  en: {
    ...
    'subagent.model.override.notice': 'You are currently running on the model `{model}`, explicitly configured by the user for this sub-agent. If asked which model you are, answer truthfully: `{model}`.',
    'subagent.model.inherited.notice': 'You are currently running on the model `{model}`, inherited from the current session. If asked which model you are, answer truthfully: `{model}`.',
  },
  zh: {
    ...
    'subagent.model.override.notice': '你当前运行在用户为该子代理显式配置的模型 `{model}` 上。如果被问到你是什么模型，请如实回答：`{model}`。',
    'subagent.model.inherited.notice': '你当前运行在模型 `{model}` 上（继承自当前会话）。如果被问到你是什么模型，请如实回答：`{model}`。',
  }
} as const;
